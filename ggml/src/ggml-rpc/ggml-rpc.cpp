...
enum rpc_cmd {
    ...
    RPC_CMD_PREFETCH_CANCEL,   // (batch cancel) drop this server's queued background pushes + abort the in-progress one at its next chunk -- a staged batch whose pressure cleared stops wasting airtime
    RPC_CMD_COUNT,
};
...
struct rpc_msg_persist_bind_rsp {
    uint64_t remote_ptr;   // 0 = no resident buffer (client must alloc + upload as normal)
    uint64_t remote_size;
};

struct rpc_msg_persist_register_req {
    uint64_t model_key;
    uint64_t remote_ptr;
};

struct rpc_msg_persist_detach_req {
    uint64_t model_key;
    uint64_t remote_ptr;
};
...
// (batch cancel) drop everything this server has queued for background push; the in-progress
// (possibly paced) transfer aborts at its next chunk boundary. Safe because the client only frees
// the staged destination buffers after the inflight counter reaches zero.
struct rpc_msg_prefetch_cancel_req {
    uint8_t dummy;
};
struct rpc_msg_prefetch_cancel_rsp {
    uint32_t dropped;   // queued jobs discarded (the in-progress one aborts asynchronously)
};
...
struct ggml_backend_rpc_buffer_context {
    ...
    // at several sites (a default-init would break that with the atomic member), so these trailing
    // members are value-initialized (0 / false) by aggregate init and set explicitly where needed.
    uint64_t                  persist_key;
    bool                      skip_upload;
};
...
// persist_end so they survive this process's teardown. KV/activation allocs happen AFTER the
// window (g_rmodel_active=false) -> normal path.
static bool rpc_xpersist_enabled() {
    static const bool on = rpc_opt_enabled() && getenv("RPC_PERSIST") != nullptr;
    return on;
}
static bool rpc_xpersist_dbg() {
...
static void ggml_backend_rpc_buffer_free_buffer(ggml_backend_buffer_t buffer) {
    ...
    if (ctx->persist_key != 0) {
        ...
        bool dok = send_rpc_cmd(ctx->sock, RPC_CMD_PERSIST_DETACH, &dreq, sizeof(dreq), nullptr, 0);
        GGML_ASSERT(dok);
        if (rpc_xpersist_dbg()) {
            GGML_LOG_INFO("[persist] client DETACH remote_ptr=%" PRIx64 " (kept resident)\n", ctx->remote_ptr);
        }
        delete ctx;
        return;
    }
...
static void ggml_backend_rpc_buffer_set_tensor(ggml_backend_buffer_t buffer, ggml_tensor * tensor, const void * data,
    ...
    if (ctx->skip_upload) {
        if (rpc_xpersist_dbg()) {
            GGML_LOG_INFO("[persist] client skip upload for %s (resident)\n", tensor->name);
        }
        return;
    }
...
static ggml_backend_buffer_t ggml_backend_rpc_buffer_type_alloc_buffer(ggml_backend_buffer_type_t buft, size_t size) {
    ...
    uint64_t rmodel_key    = 0;
    if (rpc_xpersist_enabled()) {
        std::lock_guard<std::mutex> lock(g_rmodel_mtx);
        rmodel_active = g_rmodel_active;
        rmodel_key    = g_rmodel_key;
    }
    if (rmodel_active) {
        rpc_msg_persist_bind_req breq{ rmodel_key, (uint64_t) size };
        rpc_msg_persist_bind_rsp brsp{ 0, 0 };
        bool bok = send_rpc_cmd(sock, RPC_CMD_PERSIST_BIND, &breq, sizeof(breq), &brsp, sizeof(brsp));
        GGML_ASSERT(bok);
        if (brsp.remote_ptr != 0) {
            auto * ctx = new ggml_backend_rpc_buffer_context{ sock, nullptr, brsp.remote_ptr };
            ctx->persist_key = rmodel_key;
            ctx->skip_upload = true;
            if (rpc_xpersist_dbg()) {
                GGML_LOG_INFO("[persist] client BIND HIT size=%zu -> resident (skip alloc+upload)\n", size);
            }
            return ggml_backend_buffer_init(buft, ggml_backend_rpc_buffer_interface, ctx, brsp.remote_size);
        }
    }
    ...
    if (response.remote_ptr != 0) {
        ...
        if (rmodel_active) {
            ctx->persist_key = rmodel_key;
            std::lock_guard<std::mutex> lock(g_rmodel_mtx);
            g_rmodel_pending.emplace_back(sock, response.remote_ptr);
        }
        ggml_backend_buffer_t buffer = ggml_backend_buffer_init(
            buft, ggml_backend_rpc_buffer_interface, ctx, response.remote_size);
        return buffer;
    }
    return nullptr;
}
...
class rpc_server {
  public:
    ...
    bool prefetch_cancel(const rpc_msg_prefetch_cancel_req & request, rpc_msg_prefetch_cancel_rsp & response);
  ...
  private:
    ...
    std::atomic<uint32_t>                   prefetch_cancel_gen{0};         // bumped by PREFETCH_CANCEL; in-progress push aborts when its captured gen goes stale
...
static uint64_t rpc_persist_max_bytes() {
    static const uint64_t cap = [] {
        const char * e  = getenv("RPC_PERSIST_MAX_GB");
        double       gb = e ? atof(e) : 8.0;
        if (gb <= 0.0) {
...
void rpc_server::persist_bind(const rpc_msg_persist_bind_req & request, rpc_msg_persist_bind_rsp & response, uint64_t conn_id) {
    ...
    if (it != persist_registry.end()) {
        for (auto & rb : it->second) {
            if (rb.claimed_by == 0 && rb.size == request.size) {
                ...
                if (rpc_persist_dbg()) {
                    GGML_LOG_INFO("[persist] BIND HIT key=%016" PRIx64 " size=%" PRIu64 " -> ptr=%" PRIx64 "\n",
                                  request.model_key, request.size, response.remote_ptr);
                }
                return;
            }
        }
    ...
    if (rpc_persist_dbg()) {
        GGML_LOG_INFO("[persist] BIND MISS key=%016" PRIx64 " size=%" PRIu64 "\n", request.model_key, request.size);
    }
}
...
bool rpc_server::persist_register(const rpc_msg_persist_register_req & request, uint64_t conn_id) {
    ...
    auto total_bytes = [&]() {
        uint64_t t = 0;
        for (auto & kv : persist_registry) { for (auto & rb : kv.second) { t += rb.size; } }
        return t;
    };
    while (total_bytes() > cap) {
        ...
        bool     found    = false;
        for (auto & kv : persist_registry) {
            if (kv.first == request.model_key) { continue; }
        ...
        if (!found) { break; }  // nothing evictable (all in use) -> accept being over cap
        auto vit = persist_registry.find(victim);
        uint64_t freed = 0;
        for (auto & rb : vit->second) {
            freed += rb.size;
            ggml_backend_buffer_free(rb.buffer);
            buffers.erase(rb.buffer);
        }
        persist_registry.erase(vit);
        if (rpc_persist_dbg()) {
            GGML_LOG_INFO("[persist] EVICT key=%016" PRIx64 " freed=%" PRIu64 " bytes (LRU, over %.1f GB cap)\n",
                          victim, freed, cap / (1024.0 * 1024.0 * 1024.0));
        }
    }
    return true;
}
...
void rpc_server::do_prefetch_transfer(const prefetch_job & job) {
    ...
    const uint32_t gen0 = prefetch_cancel_gen.load();
    bool ok = true;
    for (size_t off = 0; off < size || (size == 0 && off == 0); off += chunk) {
        if (prefetch_cancel_gen.load() != gen0) {
            if (dbg) { GGML_LOG_INFO("[prefetch] push CANCELLED at %zu/%zu B\n", off, size); }
            break;  // batch cancelled -- stop wasting airtime; dst buffer is about to be freed
        }
...
// (batch cancel) discard the queued pushes and tell the in-progress one to stop at its next chunk.
// The inflight counter drains to zero as usual (dropped jobs here, the aborted one when it exits),
// so the client's readiness probe / barrier semantics are unchanged.
bool rpc_server::prefetch_cancel(const rpc_msg_prefetch_cancel_req & /*request*/, rpc_msg_prefetch_cancel_rsp & response) {
    prefetch_cancel_gen.fetch_add(1);
    std::lock_guard<std::mutex> lk(prefetch_mutex);
    response.dropped = (uint32_t) prefetch_q.size();
    while (!prefetch_q.empty()) {
        prefetch_q.pop();
        prefetch_inflight--;
    }
    prefetch_cv.notify_all();
...
static void rpc_serve_client(rpc_server & server, sockfd_t sockfd, size_t free_mem, size_t total_mem) {
    ...
    while (true) {
        ...
        switch (cmd) {
            ...
            case RPC_CMD_PREFETCH_CANCEL:
                {
                    rpc_msg_prefetch_cancel_req request;
                    if (!recv_msg(sockfd, &request, sizeof(request))) {
                        return;
                    }
                    rpc_msg_prefetch_cancel_rsp response;
                    if (!server.prefetch_cancel(request, response)) {
                        return;
                    }
                    if (!send_msg(sockfd, &response, sizeof(response))) {
                        return;
            ...
            case RPC_CMD_PERSIST_REGISTER:
                {
                    rpc_msg_persist_register_req request;
                    if (!recv_msg(sockfd, &request, sizeof(request))) {
                        return;
                    }
            ...
            case RPC_CMD_PERSIST_DETACH:
                {
                    rpc_msg_persist_detach_req request;
                    if (!recv_msg(sockfd, &request, sizeof(request))) {
                        return;
                    }
                    if (!server.persist_detach(request)) {
                        return;
                    }
                    if (!send_msg(sockfd, nullptr, 0)) {
                        return;
...
static void ggml_backend_rpc_persist_begin(uint64_t model_key) {
    ...
    std::lock_guard<std::mutex> lock(g_rmodel_mtx);
    g_rmodel_active = true;
    g_rmodel_key    = model_key;
    g_rmodel_pending.clear();
    if (rpc_xpersist_dbg()) {
        GGML_LOG_INFO("[persist] persist_begin key=%016" PRIx64 "\n", model_key);
    }
}

static void ggml_backend_rpc_persist_end() {
    if (!rpc_xpersist_enabled()) {
        return;
    }
    std::vector<std::pair<std::shared_ptr<socket_t>, uint64_t>> pending;
    uint64_t key = 0;
    {
        std::lock_guard<std::mutex> lock(g_rmodel_mtx);
        pending.swap(g_rmodel_pending);
        key             = g_rmodel_key;
        g_rmodel_active = false;
        g_rmodel_key    = 0;
    }
    for (auto & p : pending) {
        rpc_msg_persist_register_req rreq{ key, p.second };
        bool ok = send_rpc_cmd(p.first, RPC_CMD_PERSIST_REGISTER, &rreq, sizeof(rreq), nullptr, 0);
        GGML_ASSERT(ok);
    }
    if (rpc_xpersist_dbg()) {
        GGML_LOG_INFO("[persist] persist_end: registered %zu fresh weight buffers under key=%016" PRIx64 "\n",
                      pending.size(), key);
    }
}
...
// (batch cancel) drop the queued pushes on the server holding `t` and abort its in-progress one at
// the next chunk. Returns queued jobs dropped (-1 = not an RPC tensor / failed).
static int ggml_backend_rpc_prefetch_cancel(const ggml_tensor * t) {
    ...
    rpc_msg_prefetch_cancel_req req;  req.dummy = 0;
    rpc_msg_prefetch_cancel_rsp rsp;  rsp.dropped = 0;
    if (!send_rpc_cmd(ctx->sock, RPC_CMD_PREFETCH_CANCEL, &req, sizeof(req), &rsp, sizeof(rsp))) {
        return -1;
    }
    return (int) rsp.dropped;
}
...
static void * ggml_backend_rpc_get_proc_address(ggml_backend_reg_t reg, const char * name) {
    ...
    if (strcmp(name, "ggml_backend_rpc_prefetch_cancel") == 0) {
        return (void *) ggml_backend_rpc_prefetch_cancel;
    }
    ...
    if (strcmp(name, "ggml_backend_rpc_persist_begin") == 0) {
        return (void *) ggml_backend_rpc_persist_begin;
    }
    if (strcmp(name, "ggml_backend_rpc_persist_end") == 0) {
        return (void *) ggml_backend_rpc_persist_end;
    }
    return NULL;
